...
void mlp_cpu_reference(const std::vector<int>& layers,
    ...
    for (size_t layer = 0; layer < layers.size() - 1; ++layer) {
        int in_dim = layers[layer]; 
        int out_dim = layers[layer + 1]; 
...
int main(int argc, char** argv) {
    ...
    check_cuda(cudaMalloc(&d_input, input_elems * sizeof(float)), "alloc d_input");
    check_cuda(cudaMalloc(&d_weights, weight_cursor * sizeof(float)), "alloc d_weights");
    check_cuda(cudaMalloc(&d_biases, bias_cursor * sizeof(float)), "alloc d_biases");

    size_t max_workspace_size = 0;
    for (int dim : opt.layers) {
        max_workspace_size = std::max(max_workspace_size, (size_t)batch * dim);
    }
    check_cuda(cudaMalloc(&d_workspace_a, max_workspace_size * sizeof(float)), "alloc d_ws_a");
    check_cuda(cudaMalloc(&d_workspace_b, max_workspace_size * sizeof(float)), "alloc d_ws_b");

    check_cuda(cudaMemcpy(d_input, h_input.data(), input_elems * sizeof(float), cudaMemcpyHostToDevice), "cp d_input");
    check_cuda(cudaMemcpy(d_weights, h_weights.data(), weight_cursor * sizeof(float), cudaMemcpyHostToDevice), "cp d_weights");
    check_cuda(cudaMemcpy(d_biases, h_biases.data(), bias_cursor * sizeof(float), cudaMemcpyHostToDevice), "cp d_biases");
    ...
    for (int i = 0; i < num_iters; ++i) {
        ...
        // Reset input for the start of the MLP
        cudaMemcpyAsync(cur_ws_a, d_input, input_elems * sizeof(float), cudaMemcpyDeviceToDevice, stream);
        ...
        for (int layer = 0; layer < num_layers; ++layer) {
            ...
            if (opt.impl == "baseline") {
                /* TODO(student): offset into d_weights based on layer. Pointer arithmetic to jump to correct memory address. */
                /* TODO(student): offset into d_biases based on layer. Pointer arithmetic to jump to correct memory address. */
                launch_bias_add(d_b, cur_ws_b, shape, stream);
                launch_activation(opt.activation, cur_ws_b, shape, stream);
            } else if (opt.impl == "activation_fused") {
                launch_fused_bias_activation(d_b, opt.activation, cur_ws_b, shape, stream);
    ...
    check_cuda(cudaEventSynchronize(stop), "sync stop");
    check_cuda(cudaEventElapsedTime(&total_elapsed_ms, start, stop), "elapsed");
    
    // Average time per forward pass
    float elapsed_ms = total_elapsed_ms / num_iters;

    /* TODO(student): copy final activations back to h_output. */
    // Note: workspace was swapped at the end of the loop, result is in d_workspace_a if num_layers is odd/even
    // Actually, std::swap at end of loop means the final output is in d_workspace_a.
    cudaMemcpyAsync(h_output.data(), d_workspace_a, output_elems * sizeof(float), cudaMemcpyDeviceToHost, stream);
    cudaStreamSynchronize(stream);
